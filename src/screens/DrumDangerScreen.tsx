...
export const DrumDangerScreen: React.FC = () => {
  ...
  const penaltyItems = chests.filter(c => c.type === type);
  // Если нет настроенных наказаний, временно показываем награды, чтобы барабан не был пустым
  const items = penaltyItems.length > 0 ? penaltyItems : chests.filter(c => c.type === 'reward');
